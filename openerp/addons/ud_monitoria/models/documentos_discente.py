...
class DocumentosDiscente(osv.Model):
    ...
    def ativo(self, cr, uid, ids, campo, args, context=None):
        res = {}
        for doc in self.browse(cr, uid, ids, context):
            res[doc.id] = doc.state != "desligado" and doc.disciplina_id.is_active
        return res

    def update_ativo(self, cr, uid, ids, context=None):
        if self._name == "ud.monitoria.disciplina":
            return self.pool.get("ud.monitoria.documentos.discente").search(cr, uid, [("disciplina_id", "in", ids)])
        return ids
    ...
    _columns = {
        ...
        "is_active": fields.function(ativo, type="boolean", string="Ativo?",
                                     store={"ud.monitoria.disciplina": (update_ativo, ["is_active"], 10),
                                            "ud.monitoria.documentos.discente": (update_ativo, ["state"], 10)}),
    ...
    def create(self, cr, uid, vals, context=None):
        if not vals.get("dados_bancarios_id", False) and vals.get("inscricao_id", False):
            ...
            vals["dados_bancarios_id"] = insc.dados_bancarios_id
