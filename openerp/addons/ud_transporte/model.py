...
class transporte_solicitacao(osv.osv):
    ...
    _columns = {
                ...
                'state':fields.selection([
                      ...
                      'Status', required=True),
                'data_hora_saida':fields.char(u'Data de saída', required=True),
                'data_hora_chegada':fields.char(u'Previsão de chegada', required=True),
                'solicitante_email':fields.function(_email, string="E-mail",type="char"),
                'solicitante_telefone':fields.function(_telefone, string='Telefone',type="char"),
                'solicitante_matricula':fields.function(_matricula, string=u'Matrícula',type="char"),
    ...
    def busca_dados (self, cr, uid, ids, field, args, context):
        ...
        if field == "solicitante_telefone":
            print field
            if self.browse(cr, uid, ids, context)[0].solicitante.mobile_phone != None or self.browse(cr, uid, ids, context)[0].solicitante.work_phone != None:
                return {dados.id : self.browse(cr, uid, ids, context)[0].solicitante.mobile_phone or self.browse(cr, uid, ids, context)[0].solicitante.work_phone or "Não cadastrado"}
            else:
                raise except_orm(u"Telefone não encontrado", u"É necessário ao menos um telefone cadastrado")
        elif field == "solicitante_email":
            if self.browse(cr, uid, ids, context)[0].solicitante.work_email != None:
                return {dados.id:self.browse(cr, uid, ids, context)[0].solicitante.work_email}
            else:
                raise except_orm(u"E-mail não encontrado", u"É necessário existir um e-mail cadastrado")
        elif field == "solicitante_matricula":
            if self.browse(cr, uid, ids, context)[0].solicitante.papel_ids[0].matricula != None:
                return {dados.id:self.browse(cr, uid, ids, context)[0].solicitante.papel_ids[0].matricula}
            else:
    ...
    def converteData(self, dataS):
        ...
        print "Partes data hora"
        print partes_data
        print partes_hora
        data = datetime(int(partes_data[2]),int(partes_data[1]),int(partes_data[0]),
                        int(partes_hora[0]),int(partes_hora[1]), int(partes_hora[2]))
        print data
        return data
    
    def formata_data_hora (self, data_hora):
        data = data_hora[:10]
        data = data.replace("/", "-")
        hora = data_hora[14:20]
        hora = hora.replace("h ", ":")
        return data + " " + hora + ":00"
    ...
    def _check_data_chegada(self, cr, uid, ids,context=None):
        ...
        '''
        try:
            obj_solicitacao = self.browse(cr,uid,ids)[0]
            solicitante_data_hora_chegada = obj_solicitacao["data_hora_chegada"]
            solicitante_data_hora_chegada = self.formata_data_hora(solicitante_data_hora_chegada)
            solicitante_data_hora_saida = obj_solicitacao["data_hora_saida"]
            solicitante_data_hora_saida = self.formata_data_hora(solicitante_data_hora_saida)
    ...
    def _check_data(self, cr, uid, ids, context=None):
        try:
            solicitacao = self.browse(cr, uid, ids, context)[0]
            solicitante_data_hora_saida = solicitacao["data_hora_saida"]
            solicitante_data_hora_saida = self.formata_data_hora(solicitante_data_hora_saida)
            data_inicial = self.converteData(solicitante_data_hora_saida)
            if data_inicial < datetime.today():
                return False
            return True
        except:
            return False
    ...
    _constraints = [
                    (_check_data_chegada, 'Verifique se a data e a hora estão corretas A data de previsão de chegada deve ser posterior a data de saída', ['solicitante_data_hora_saida']),
                    (_check_data, 'Verifique se a data e a hora estão corretas A data deve ser posterior a data corrente', ['solicitante_data_hora_saida']),
                    ]
...
class viagem (osv.osv):
    ...
    _columns = {
                ...
                'data_solicitacao':fields.char('Data de solicitação', states={"concluida":[("readonly", True)]}),
                'data_solicitacao_chegada':fields.char('Previsão de chegada', states={"concluida":[("readonly", True)]}),
    ...
    def _formata_data (self, dataS):
        dataS = self.formata_data_hora(dataS)
        partes_data = dataS[:10].split("-")
        partes_hora = dataS[11:].split(":")
        data = datetime(int(partes_data[2]),int(partes_data[1]),int(partes_data[0]),
                        int(partes_hora[0]),int(partes_hora[1]), int(partes_hora[2]))
