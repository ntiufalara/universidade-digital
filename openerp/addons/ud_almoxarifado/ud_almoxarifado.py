...
class ud_almoxarifado_produto_qtd (osv.osv):
    ...
    def _valida_quantidade(self, cr, uid, ids, context=None):# método usado para fazer a validação
        ...
        for produto_qtd in self.browse(cr, uid, ids, context=context):
            if produto_qtd.quantidade < 1:
                return False
            estoque = estoque_model.search(cr, uid, [("produto_id", "=", produto_qtd.produto_id.id),("quantidade", "<=", produto_qtd.quantidade)], context=context)
            if (not estoque):
                return False
        return True
    ...
    def _estoque(self, cr, uid, ids, campo, args, context = None):
        ...
        for produto_qtd in self.read(cr, uid, ids, ['produto_id','id'], load = '_classic_write', context=context):
            quantidade = estoque_model._calcula_quantidade(cr, uid, estoque_model.search(cr, uid, [('produto_id', '=', produto_qtd.get('produto_id'))], context=context), None, None, context=context), 
            res[produto_qtd.get('id')] = quantidade[0].values()[0]
        return res
    
    _columns = {
                
        'produto_id': fields.many2one('ud.almoxarifado.produto', u'Produto', domain = "[('categoria_id','=', categoria_id)]",required=True, ondelete='cascade'),
        ...
        'solicitacao_id':fields.many2one('ud.almoxarifado.solicitacao', 'solicitacao', invisible=True, ondelete='cascade'),
    }
    
    _rec_name = 'produto_id'
    
    _sql_constraints = [
        ('produto_solicitacao_uniq', 'unique (solicitacao_id,produto_id)', u'Não pode solicitar o memo produto na mesma solicitação'),
    ]
    ...
    def onchange_produto(self, cr, uid, ids, produto, context=None):
        estoque_model = self.pool.get('ud.almoxarifado.estoque')
        quantidade = estoque_model._calcula_quantidade(cr, uid, estoque_model.search(cr, uid, [('produto_id', '=', produto)], context=context), None, None, context=context), 
    ...
    def create(self, cr, uid, vals, context={}):
        ...
        estoque_model.write(cr, uid, estoque[0], {'saida_ids': [(0, 0, {'data_saida':datetime.utcnow().strftime('%Y-%m-%d'),
    ...
    def restaurar_produtos(self, cr, uid, ids, context=None):
        ...
        for produto in self.browse(cr, uid, ids, context=context):
            ...
            estoque_model.write(cr, uid, estoque[0], {'entrada_ids': [(0, 0, {'data_entrada': datetime.utcnow().strftime('%Y-%m-%d'),
...
class ud_almoxarifado_solicitacao (osv.osv):
    ...
    def _get_solicitante(self, cr, uid, ids, field, arg, context = None):
        if isinstance(ids, (int, long)): # verifica se o id é um inteiro ou longo, se for ele colocará o inteiro em uma lista
            ids = [ids]
        domain = [('user_id', '=', uid)]
    ...
    _columns = {
        'produtos_ids':fields.one2many('ud.almoxarifado.produto.qtd','solicitacao_id', string=u'Produtos',required=True),
...
class ud_almoxarifado_fornecedor (osv.osv):
    ...
    _ESTADOS = [('ac', u'AC'), ('al', u'AL'), ('ap', u'AP'), ('am', u'AM'), ('ba',u'BA'), ('ce',u'CE'), ('df',u'DF'), ('es',u'ES'),
                ('go',u'GO'), ('ma',u'MA') , ('mg',u'MG'), ('ms',u'MS'), ('mt',u'MT'), ('pa',u'PA'), ('pe',u'PE'), ('pi',u'PI'),
                ('pr',u'PR'), ('rj',u'RJ'), ('rn',u'RN'), ('ro',u'RO'), ('rr',u'RR'), ('rs',u'RS'), ('sc',u'SC'), ('se',u'SE'),
                ('sp',u'SP'), ('to', u'TO')]
...
#     _order = 'name asc' 
    ...
    _columns = {
        'produto_id':fields.many2one('ud.almoxarifado.produto',string=u'Produto', required=True, readonly = True, ondelete="restrict"),
        'observacao': fields.related('produto_id', 'observacao', string=u'Observação', type='text', readonly = True),
        'categoria_id':fields.related('produto_id', 'categoria_id', type = 'many2one', relation = 'ud.almoxarifado.categoria', string = u'Categoria', readonly = True),
        ...
        'quantidade': fields.function(_calcula_quantidade, type="integer", method=True, string= u'Quantidade', 
                                      store=False), #atualiza o modelo "ud.almoxarifado.estoque' sempre que os campos dentro da lista forem atualizados ele retorna a lista de ids que precisam serem atualizados.
    ...
    _constraints = [
        (_valida_quantidade, u'A quantidade não pode ser menor do que 0', ['\nQuantidade']),
...
class ud_almoxarifado_entrada(osv.Model):
    ...
    _columns = {
                ...
                'quantidade' : fields.integer(u'Quantidade', required=True),
                'tipo' : fields.selection([("fornecedor", u"Fornecedor"),("estorno",u"Estorno"),("devolucao", u"Devolução")], u"Tipo"),
                'estoque_id' : fields.many2one('ud.almoxarifado.estoque', u'Estoque', invisible=True, ondelete='cascade'),
                'fornecedor_id' : fields.many2one('ud.almoxarifado.fornecedor',u'Fornecedor', readonly=True, ondelete='restrict')
    }
