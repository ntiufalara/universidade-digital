...
class patrimonio_bem(osv.osv):
    ...
    _verifica_os = lambda self, cr, uid, ids, field, args, context: self.verifica_os(cr, uid, ids, field, args, context)
    ...
    _columns = {
        ...
        u'espaco_campus': fields.many2one('ud.campus', u"Campus Lotado", ondelete="cascade", required=True),
        u'espaco_polo': fields.many2one('ud.polo', u"Polo Lotado", ondelete="cascade", required=True),
        ...
        u'perm_solicitacao': fields.function(_verifica_os, type="boolean", string="Permitir Solicitação"),
    }
    ...
    def verifica_os(self, cr, uid, ids, field, args, context):
        """
        Verifica se existe uma solicitação aberta
        :param cr:
        :param uid:
        :param ids:
        :param field:
        :param args:
        :param context:
        :return: valor_do_campo: dict
        """
        this = self.browse(cr, uid, ids)[0]
        ret = True
        for solicitacao in this.solicitacao_os:
            if solicitacao.state == 'nova':
                ret = False
        return {this.id: ret}
