...
#                 "email":lambda self,cr,uid,c: self.email(cr, uid, c),
    ...
    def busca_dados (self, cr, uid, ids, field, args, context):
        ...
        if field == "matricula":
            ...
            print field
            if self.browse(cr, uid, ids, context)[0].solicitante_id.mobile_phone != None or self.browse(cr, uid, ids, context)[0].solicitante_id.work_phone != None:
                return {dados.id : self.browse(cr, uid, ids, context)[0].solicitante_id.mobile_phone or self.browse(cr, uid, ids, context)[0].solicitante_id.work_phone}
            else:
    ...
    def onchange_solicitante(self, cr, uid,ids, solicitante, context=None):
        try:
            ...
            return {'value': {'matricula': papel.matricula, 'telefone':pessoa.mobile_phone or pessoa.work_phone, 'email':pessoa.work_email,'setor_id': setor_curso}}
        except IndexError:
            raise except_orm("Pessoa não possui papel".decode("UTF-8"), "Não há papel associada a esse usuário".decode("UTF-8"))
