...
class ud_nova_solicitacao(osv.osv):
    ...
    status = (("nova", "Solicitação enviada"), ("atribuida", 'Análise'), ("aprovada", "Encaminhado p/ Execução"),
              ("execucao", "Em Execução"), ("finalizada", "Finalizada"), ("cancelada", "Cancelada"),)
    ...
    _columns = {
        ...
        "manutencao": fields.selection((("equip", "Equipamento"), ("pred", "Predial"), ("limp", "Limpeza"),
                                        ("movmat", "Movimentação de Materiais")), "Manutenção", required=True),

        "mov_denominacao": fields.char("Denominação"),

        "equipamento": fields.selection((("elt", "Elétricos"), ("hid", "Hidráulicos")), "Equipamentos"),
        "eq_eletricos": fields.selection((('apref', "Refrigeradores"), ("info", u"Informática"), ("outros", "Outros")),
                                         "Tipo de equipamento"),
        ...
        "refrigeradores": fields.selection(
                (("arc", "Ar Condicionado"), ("outrod", "Outros (Geladeiras, Bebedouros etc)")),
                "Tipo de refrigerador"),
        "ref_tipo": fields.selection((("split", "Split"), ("janela", "Janela")), "Tipo de ar condicionado"),
        "predial": fields.selection(
                (('inst', 'Instalações'), ('alv', 'Alvenaria / Esquadrias / Metálicas / Divisórias')),
                "Manutenção predial"),
    ...
    def limpa_mnt(self, cr, uid, ids):
        ...
        método on_change
        '''
        campos = ["mov_denominacao", "equipamento", "eq_eletricos", "equip_denominacao", "modelo", "marca",
                  "refrigeradores", "ref_tipo", "predial", "instalacoes", "local_camp_destino", "local_polo_destino",
                  "local_bloco_destino", "local_sala_destino"]
        valores = {}
        for campo in campos:
            valores[campo] = ""
    ...
    def onchange_solicitante(self, cr, uid, ids, solicitante, context=None):
        try:
            ...
            pessoa = pessoa[0]
            papel = self.pool.get('ud.perfil').browse(cr, uid, pessoa.papel_ids[0].id)

            setor_curso = papel.ud_setores.name or papel.ud_cursos.name

            return {'value': {'matricula': papel.matricula, 'telefone': pessoa.mobile_phone or pessoa.work_phone,
                              'email': pessoa.work_email, 'setor_id': setor_curso}}
        except IndexError:
            raise except_orm("Pessoa não possui papel".decode("UTF-8"),
                             "Não há papel associada a esse usuário".decode("UTF-8"))
    ...
    def matricula(self, cr, uid, c):
        ...
        pessoa = pessoa[0]
        papel = self.pool.get('ud.perfil').browse(cr, uid, pessoa.papel_ids)
        matricula = papel.matricula
        if matricula:
            return matricula
        else:
            raise except_orm("Pessoa não localizada".decode("UTF-8"),
                             "É necessário que a pessoa tenha um papel para realizar solicitações".decode("UTF-8"))
    ...
    def telefone(self, cr, uid, c):
        ...
        if pessoa:
            ...
            raise except_orm("Pessoa não possui telefone".decode("UTF-8"),
                             "É necessário possuir um telefone cadastrado para realizar solicitações".decode("UTF-8"))
    ...
    def setor(self, cr, uid, c):
        ...
        if pessoa.papel_ids.setor.ud_setores:
            return pessoa.papel_ids.setor.ud_setores[0].name
        elif pessoa.papel_ids.setor.ud_cursos:
            return pessoa.papel_ids.setor.ud_cursos[0].name
        raise except_orm("Pessoa não possui papel".decode("UTF-8"),
                         "É necessário que a pessoa tenha um papel para realizar solicitações".decode("UTF-8"))
    ...
    def email(self, cr, uid, c):
        ...
        if email:
            ...
            raise except_orm("Pessoa não possui e-mail".decode("UTF-8"),
                             "É necessário que a pessoa tenha um email cadastrado para realizar solicitações".decode(
                                 "UTF-8"))
    ...
    def execucao(self, cr, uid, ids, ctx):
        ...
        if slc.state == "aprovada" and slc.execucao:
            return self.write(cr, uid, ids,
                              {"state": "execucao", "execucao": slc.execucao, "data_execucao": fields.datetime.now()},
                              context=ctx)
    ...
    def finalizar(self, cr, uid, ids, ctx):
        ...
        if slc.state == "execucao" and slc.finalizar and slc.tempo_gasto:
            return self.write(cr, uid, ids,
                              {"state": "finalizada", "finalizar": slc.finalizar, "tempo_gasto": slc.tempo_gasto},
                              context=ctx)
